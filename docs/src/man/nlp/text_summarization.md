...
```
If we were to use the root instead of lemma, the following would be the result
```@setup def
using Pkg
...
roots = root.(feats)
forms1 = select(crpsnew, :form)
forms1[.!ismissing.(roots)] = roots[.!ismissing.(roots)]
crpsnew = transform(crpsnew, :form => forms1)
crpsnew = CorpusData(crpsnew)
roots_vrs = verses(crpsnew)
vrs = QuranTree.normalize.(dediac.(roots_vrs))
crps1 = Corpus(StringDocument.(vrs))
update_lexicon!(crps1)
update_inverse_index!(crps1)
m1 = DocumentTermMatrix(crps1)
...
idx = sortperm(vec(p), rev=true)[1:10]
verse_nos = verses(CorpusData(crpstbl), number=true, start_end=false)
function summary(crpsdata, verse_nos)
    verse_out = String[];
    chapter = Int64[];
    verse = Int64[];
    for v in verse_nos
    ...
    return verse_out, chapter, verse
end

verse_out, chapter, verse = summary(crpsdata, verse_nos)
```
```@repl def
using PrettyTables
...
```
Or if we were to use Chapter 5 (The Feast), the following verses would best summarize the said chapter:
```@setup hij
using Pkg
Pkg.add("PrettyTables")
Pkg.add("JuliaDB")
...
crpstbl = filter(t -> preprocess(t.features), crpsdata[5].data) # chapter 5
crpsnew = deepcopy(crpstbl)
feats = select(crpsnew, :features)
feats = parse.(Features, feats)
lemmas = lemma.(feats)
...
crpsnew = transform(crpsnew, :form => forms1)
crpsnew = CorpusData(crpsnew)
lemmas_vrs = verses(crpsnew)
vrs = QuranTree.normalize.(dediac.(lemmas_vrs))
crps1 = Corpus(StringDocument.(vrs))
update_lexicon!(crps1)
update_inverse_index!(crps1)
m1 = DocumentTermMatrix(crps1)
